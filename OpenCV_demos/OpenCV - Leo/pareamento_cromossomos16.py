...
# Garantir que temos 23 pares
num_pares = len(pares_selecionados)

# Se houver menos de 23 pares, ajusta para o número disponível
num_linhas = 5  # Ajuste para ter 5 linhas, se necessário
...
# Loop para exibir os pares de forma organizada
indice_par = 0
for linha in range(num_linhas):
    for coluna in range(num_colunas):
        if indice_par < num_pares:
            ...
            # Extrair apenas a área do cromossomo (sem o fundo)
            cromossomo1 = imagem[y1:y1 + h1, x1:x1 + w1]
            cromossomo2 = imagem[y2:y2 + h2, x2:x2 + w2]

            # Redimensiona os cromossomos para manter a proporção
            fator_proporcao1 = altura_par / float(h1)
            largura_redimensionada1 = int(w1 * fator_proporcao1)
            cromossomo1_redimensionado = cv2.resize(cromossomo1, (largura_redimensionada1, altura_par))

            fator_proporcao2 = altura_par / float(h2)
            largura_redimensionada2 = int(w2 * fator_proporcao2)
            cromossomo2_redimensionado = cv2.resize(cromossomo2, (largura_redimensionada2, altura_par))
            ...
            x_offset = coluna * (largura_redimensionada1 + largura_redimensionada2)
            y_offset = linha * altura_par

            # Insere o par na imagem final
            img_pares[y_offset:y_offset + altura_par, x_offset:x_offset + largura_redimensionada1 + largura_redimensionada2] = par_horizontal

            # Adiciona o número do par
            cv2.putText(img_pares, str(indice_par + 1), (x_offset + largura_redimensionada1, y_offset + altura_par // 2),
