...
# Tamanho da imagem para cada par, agora mantendo a proporção original
img_altura = imagem.shape[0]
img_largura = imagem.shape[1]

# Criar uma imagem final com fundo branco
largura_total = img_largura * num_colunas
altura_total = img_altura * num_linhas
...
for linha in range(num_linhas):
    for coluna in range(num_colunas):
        if indice_par < len(pares_selecionados):
            ...
            # Ajusta os cromossomos para o tamanho adequado
            altura_max = img_altura // num_linhas
            largura_max = img_largura // num_colunas

            cromossomo1_redimensionado = cv2.resize(cromossomo1, (largura_max, altura_max))
            cromossomo2_redimensionado = cv2.resize(cromossomo2, (largura_max, altura_max))
            ...
            x_offset = coluna * largura_max * 2
            y_offset = linha * altura_max

            # Insere o par na imagem final
            img_pares[y_offset:y_offset + altura_max, x_offset:x_offset + largura_max * 2] = par_horizontal

            # Adiciona o número do par na imagem
            cv2.putText(img_pares, str(indice_par + 1), (x_offset + largura_max, y_offset + altura_max // 2),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 1, cv2.LINE_AA)

            indice_par += 1

# Salva a imagem final
cv2.imwrite("pares_de_cromossomos_final_sem_distorsao.jpg", img_pares)
